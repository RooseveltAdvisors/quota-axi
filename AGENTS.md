...
- It reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, and Alibaba Coding Plan quota windows. Alibaba uses bounded first-party console/API paths from `$ALIBABA_CODING_PLAN_COOKIE`, the exact `pi:alibaba-plan` Pi auth entry, or documented API-key aliases; unavailable usage stays explicit and never fabricates quota or token metrics. It must never route, recommend, proxy, intercept, log in, import browser cookies, or mutate provider-side state. Default quota reads may renew short-lived OAuth credentials in local auth files; `--no-refresh` and `QUOTA_AXI_NO_REFRESH=1` disable those writes.
