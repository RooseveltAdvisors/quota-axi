...
- It reports local Claude, Codex, Cursor, GitHub Copilot, Grok, and Kimi quota windows plus Alibaba Coding Plan entitlement metadata from the exact `pi:alibaba-plan` Pi auth entry. Alibaba usage is console-only, so it reports no numeric quota windows or fabricated token metrics. It must never route, recommend, proxy, intercept, log in, import browser cookies, or mutate provider-side state. Default quota reads may renew short-lived OAuth credentials in local auth files; `--no-refresh` and `QUOTA_AXI_NO_REFRESH=1` disable those writes.
