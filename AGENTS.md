...
- Grok reads `$GROK_AUTH_JSON`, inline `$GROK_AUTH`, `$GROK_AUTH_PATH`, or `$GROK_HOME/auth.json`/`~/.grok/auth.json`; when none of those selectors is set, a missing or expired default session can fall back to Pi's `xai` OAuth grant as `pi:xai`. `$GROK_AUTH_JSON` or inline `$GROK_AUTH` may coexist with independent Pi model-auth inspection, but `$GROK_AUTH_PATH` or `$GROK_HOME` pins the standalone session; Pi credentials are never sent to the consumer quota endpoint. It selects session-scoped auth instead of API-key entries and recognizes OIDC records scoped to `auth.x.ai` with `auth_mode` or `authMode` set to `oidc`.
...
`release-please-config.json` keeps the published npm `0.1.0` bootstrap baseline; the committed `.release-please-manifest.json` and `package.json` version are release-please-owned generated state, and release-please owns every version after that.
