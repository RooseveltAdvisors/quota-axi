...
- It reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, and Alibaba Coding Plan or Personal Token Plan quota windows. Alibaba source precedence is the explicit `$ALIBABA_TOKEN_PLAN_COOKIE`, the backward-compatible `$ALIBABA_CODING_PLAN_COOKIE`, the exact `pi:alibaba-plan` Pi auth entry, then documented API-key aliases. Only the Token Plan cookie calls the Token Plan console operation; it emits percentage windows with `accounting: token_plan`, while the other sources remain Coding Plan-only. Alibaba requests are bounded and unavailable usage stays explicit. It must never route, recommend, proxy, intercept, log in, import browser cookies, persist cookies or `sec_token`, or mutate provider-side state. Default quota reads may renew short-lived OAuth credentials in local auth files; `--no-refresh` and `QUOTA_AXI_NO_REFRESH=1` disable those writes.
...
- Grok reads `$GROK_AUTH_JSON`, inline `$GROK_AUTH`, `$GROK_AUTH_PATH`, or `$GROK_HOME/auth.json`/`~/.grok/auth.json`, and only when none of those is configured and the default session is missing or expired falls back to Pi's `xai` OAuth grant as the `pi:xai` source. It selects session-scoped auth instead of API-key entries and recognizes OIDC records scoped to `auth.x.ai` with `auth_mode` or `authMode` set to `oidc`.
...
- Grok cache fallback accepts only current `web` source snapshots, not legacy `api` billing-proxy entries. The default read path renews local Grok/Pi OAuth credentials with the shared lock-and-atomic-write helper; it never sends cookies, launches Grok, or retains raw responses. See [README Security Posture](README.md#security-posture).
- Kimi's credential, fallback, and transport contracts are documented in [README Security Posture](README.md#security-posture).
- Grok and Kimi OAuth renewal uses the shared `src/lib/oauth.ts` helper for first-party token POSTs, auth-file locking, and atomic `0600` writes. Codex treats an expired `id_token` as stale metadata when its access token remains valid; Codex refresh is not enabled because no stable endpoint is known, so cli-rpc remains the fallback. See [README Security Posture](README.md#security-posture).
