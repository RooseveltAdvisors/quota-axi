...
- Grok reads `$GROK_AUTH_JSON`, inline `$GROK_AUTH`, `$GROK_AUTH_PATH`, or `$GROK_HOME/auth.json`/`~/.grok/auth.json`, and only when none of those is configured and the default file is absent falls back to Pi's `xai` OAuth grant as the `pi:xai` source. It selects session-scoped auth instead of API-key entries and recognizes OIDC records scoped to `auth.x.ai` with `auth_mode` or `authMode` set to `oidc`.
