...
* merge upstream human terminal quota reporting, model evidence, effective runway, and auth-classification improvements while retaining Alibaba Coding Plan and Token Plan providers
* renew eligible local Grok and Kimi OAuth credentials on quota reads, with `--no-refresh` read-only mode

### Bug Fixes

* preserve Alibaba source precedence, authoritative usage accounting, and degraded-state reporting across provider integrations
* retain the hardened no-mistakes pull-request gate and Pi-authenticated Kimi/Grok quota paths
