...
- A provider whose window relationships are wholly unknown, or whose partial semantics have no effective availability while every reported window remains unresolved, has no combined effective percentage, pace, or runway to show. Its card replaces the headline block with a single `per-window usage · no combined bound` line and leads straight into its real per-window rows. Partially understood providers with an effective availability entry keep the effective-unknown headline. No combined headroom, pace, or runway number is invented.
