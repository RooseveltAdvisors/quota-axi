...
Alibaba Coding Plan uses `pi:alibaba-plan` as an API-key-compatible first-party credential when Alibaba accepts it. An explicit `ALIBABA_CODING_PLAN_COOKIE` (a CodexBar-compatible `Cookie` header) takes precedence and uses the international Model Studio console RPC, discovering `sec_token` from the dashboard HTML, user-info response, or cookie. If the Pi credential is unavailable, quota-axi may use `ALIBABA_CODING_PLAN_API_KEY`, `ALIBABA_QWEN_API_KEY`, or `DASHSCOPE_API_KEY`. Set `ALIBABA_CODING_PLAN_REGION=cn-beijing` for the documented China API host; international is the default.
...
| Alibaba Coding Plan    | Uses the configured cookie console path (`web`) first, then `pi:alibaba-plan` or documented API-key aliases (`api`). Reports server-defined `five_hour`, `weekly`, and `monthly` request-quota windows, reset timestamps, raw counters, plan/instance/model metadata, and multipliers when present. Credit/token-plan accounting is not treated as request quota; unavailable sources remain explicit and windowless.                                                                                                                                                  |
...
- The console path is opt-in through `$ALIBABA_CODING_PLAN_COOKIE`, a user-local semicolon-separated `Cookie` header compatible with CodexBar's manual cookie source. quota-axi does not import browser cookies, launch a browser, persist the header, persist `sec_token`, or print either value. It performs bounded dashboard/user-info discovery and posts the international form RPC with `params`, `region`, and `sec_token`.
- Source precedence is cookie, Pi credential, then the documented API-key aliases. `ALIBABA_CODING_PLAN_REGION=cn-beijing` selects the extensible China API configuration; international is the default. The API-key path sends only the first-party Alibaba request headers and JSON body documented by CodexBar.
